...
[项目 git 地址](https://github.com/alowkeyguy/webpack4.x-vue)
...
你可以尝试的`npm run lint`，你会发现`spacing 系列`、`no-new`、`.vue标签报错`等问题
...
当然这玩法不是我们这里的重点，安装 eslint-plugin-vue 来解决 vue 语法报错问题，然后装个 eslint-plugin-html 约束 vue 文件的编写规范同 html 一致

```
npm install -D eslint-plugin-vue eslint-plugin-html
...
至于 main.js 里的 new 指定给变量 disable 掉
